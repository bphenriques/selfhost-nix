{ lib, config, ... }:
let
  inherit (config) selfhost;
  rcCfg = selfhost.resourceControl;
  ...
  collectUnits =
    ...
    let
      ...
      fromServices = lib.concatMap (
        svc: lib.optionals (svc.resourceControl.slice == sliceName) svc.resourceControl.systemdServices
      ) (lib.attrValues selfhost.services);
    in
  ...
  servicesMissingUnits = lib.attrNames (
    lib.filterAttrs (
      _: svc: svc.resourceControl.slice != null && svc.resourceControl.systemdServices == [ ]
    ) selfhost.services
  );
in
{
  options.selfhost.resourceControl = {
    slices = lib.mkOption {

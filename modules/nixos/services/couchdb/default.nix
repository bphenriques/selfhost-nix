...
# model a replicating client can speak. enableSelfhostIntegration derives accounts and their databases
# from selfhost.users.
{
  config,
  lib,
...
in
{
  ...
  options.selfhost.apps.couchdb = {
    ...
    enableSelfhostIntegration = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Derive CouchDB accounts and databases from selfhost.users grants. Turn off to run CouchDB but manage its accounts yourself.";
    };
  };
  ...
  config = lib.mkMerge [
    (lib.mkIf (config.selfhost.enable && app.enable) {
      ...
      selfhost.runtimeSecrets.couchdb-admin-password.restartUnits = [
        "couchdb.service"
    ...
    (lib.mkIf (config.selfhost.enable && app.enable && app.enableSelfhostIntegration) {
      warnings =
        lib.optional (enabledUsers == { })
          "selfhost.apps.couchdb: enableSelfhostIntegration is on but no selfhost.users have services.couchdb.enable — CouchDB will have no accounts.";

      selfhost.runtimeSecrets = lib.mapAttrs' (
        uname: _:

...
let
  ...
  monitoringScopeModule = _: {
    options = {
      enable = lib.mkEnableOption "this monitoring scope" // {
        default = true;
      };
  ...
  blackboxScrapeConfigs = lib.optionals hasHealthchecks (
    let
      byModule = builtins.groupBy (s: s.healthcheck.probeModule) healthcheckedServices;
    in
    lib.mapAttrsToList (moduleName: services: {
      ...
      relabel_configs = [
        ...
        }
        {
          ...
          replacement = "127.0.0.1:${toString mon.blackboxPort}";
        }
      ];
    }) byModule
  );
  ...
  exporterPorts =
    ...
    in
    lib.concatLists (
      lib.mapAttrsToList (name: scope: fromExporters "scope/${name}" scope.exporters) (
...
in
{
  options.selfhost.monitoring = {
    ...
    prometheusPort = lib.mkOption {
      type = lib.types.port;
      default = 9090;
      description = "Prometheus listen port (localhost, behind ingress).";
    };

    blackboxPort = lib.mkOption {
      type = lib.types.port;
      default = 9116;
      description = "Blackbox-exporter listen port (localhost) for service healthchecks.";
    };

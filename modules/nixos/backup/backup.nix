...
let
  ...
  resolveHooks =
    ...
    lib.genAttrs t.services (svcName: {
      inherit (selfhostCfg.services.${svcName}.backup) package after;
    })
  ...
  mkBackupService =
    ...
    lib.nameValuePair "homelab-backup-${name}" {
      ...
      serviceConfig = hardenedServiceConfig // {
        ...
        ExecStopPost = [
          "${pkgs.findutils}/bin/find ${targetExtras name} -mindepth 1 -maxdepth 1 -exec ${pkgs.coreutils}/bin/rm -rf -- {} +"
        ];
        TimeoutStartSec = "6h";
  ...
  mkVerifyService =
    name: t:
    lib.nameValuePair "homelab-backup-${name}-verify" {
  ...
  mkTimer =
    ...
    lib.nameValuePair "homelab-backup-${name}${suffix}" {
          ...
          options = {
            ...
            globs = lib.mkOption {
              ...
              default = [
                ...
                "!.DS_Store"
                "!Thumbs.db"
                # Filesystem-managed snapshot views: each holds a full point-in-time copy of the
                # tree, so binding a share root without these walks it once per snapshot. Synology
                # exposes #snapshot; ZFS exposes .zfs once snapdir=visible is set.
                "!#snapshot"
                "!#recycle"
                "!.zfs"
              ];

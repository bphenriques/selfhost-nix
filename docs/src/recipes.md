...
**register a service the framework doesn't bundle**, doing by hand the wiring an app does for you. For
real, always-current examples see [Examples in the wild](#examples-in-the-wild).
...
in
{
  ...
  selfhost.services.miniflux = {
    ...
    access.model = "oidc";
    access.oidc.systemd.dependentServices = [ "miniflux" ]; # start after its client is provisioned
  ...
  services.miniflux = {
    ...
    config = {
      ...
      OAUTH2_REDIRECT_URL = builtins.head svc.access.oidc.callbackURLs;
      OAUTH2_CLIENT_ID_FILE = svc.access.oidc.id.file; # provisioned at boot
      OAUTH2_CLIENT_SECRET_FILE = svc.access.oidc.secret.file; # never in the Nix store
    };
  };

  # 3. Let miniflux read those files (it takes them as *_FILE env vars).
  systemd.services.miniflux.serviceConfig.SupplementaryGroups = svc.access.oidc.systemd.supplementaryGroups;
}
```

The framework owns the route, client provisioning, tile, healthcheck, and secrets, and exposes them as
derived attributes (`svc.publicUrl`, `svc.access.oidc.id.file`, …). You own the service and the few lines that
connect the two.
...
- **Secrets in-settings** (e.g. Immich): a service that takes a file *path* in its own config skips the env
  vars (`settings.oauth = { inherit (oidc.provider) issuerUrl; clientId._secret = svc.access.oidc.id.file;
  clientSecret._secret = svc.access.oidc.secret.file; }`) but still needs the supplementary group, since it reads
  those files as its own user (`SupplementaryGroups = svc.access.oidc.systemd.supplementaryGroups`).
- **Forward-auth instead of OIDC** (no SSO of its own): drop the `access.oidc` block and set
...
with this flake. It is the exhaustive catalogue this page deliberately isn't.

...
attributes **mirror the framework's top-level namespace 1:1**, so where an option lives tells you what it
touches and where to look for it:
...
object as its identity, so there is no parallel user tree to join and identity stays single-source.

...
design: **the framework wires the plumbing; it never configures a source.**
...
- `rootFolders` — library paths (storage-agnostic; the path must exist).
...
- `delayProfile` — optional; carries the protocol preference, so it stays your call with no default.
...
download clients. Its indexer list and app-sync are acquisition, and live in your config; they read the
apps' `apiKeyFile`.

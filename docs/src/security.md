...
- **Durable secret lifecycle.** `regenerateIfMissing` for disposable random secrets; `generateOnce`
  (+ `generateOnceGuard`) for data-bound keys — a key lost while its data survives is left *absent and logged*,
  never silently replaced.
- **The edge is the only public surface.** Services bind `127.0.0.1`; the reverse proxy fronts them. Access is
  gated by per-service OIDC clients (group-scoped) or `forwardAuth` — the forwardAuth middleware sets the
  identity headers from the auth response, and an assertion refuses `forwardAuth` with no active provider.
...
- **WireGuard, not public exposure** — the promoted way in.
...
- **Harden the host.** SSH, firewall baseline, kernel/account hardening, disk encryption — the host's job. The
  framework hardens *its* service sandboxes, not your machine.
...
There is no restore command on purpose: in a real disaster the host — and any wrapper it built — is gone, so
recovery is a manual runbook you must be able to run from nothing but your backups and your out-of-band keys.
...
1. **Rebuild the host** and restore the secrets backend — put the sops/age master key back first.
...
3. **`nixos-rebuild switch`** — disposable secrets regenerate, OIDC users/clients re-provision.
4. **Restore data** with rustic into each service's data location, then replay hook outputs (import dumps,
   ...
   <id> --to <dest>`; on a bare machine, reconstruct the repo string, password, and backend credentials by hand
   from your secrets store first.

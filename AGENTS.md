...
- **Per-user namespace is a 1:1 mirror** (see the Users docs chapter): a per-user option lives at `selfhost.users.<name>.<path>` mirroring its top-level `selfhost.<path>`. An app's sits at `selfhost.users.*.apps.<name>` (e.g. `apps.wireguard.devices`), a concern's at `selfhost.users.*.<concern>` (e.g. `auth.oidc`). Consumer-owned per-user config that isn't the framework's lives under `selfhost.users.*.extraConfig` (freeform, or typed by the consumer). Don't invent a flat per-user key. Mirror the path so it's discoverable.

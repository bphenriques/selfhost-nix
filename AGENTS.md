...
- **The registry submodule is composed centrally, on purpose**: `services-registry.nix` lists every `schemas/*.nix` facet, so one file spells out the whole service contract. A concern could instead extend the submodule from its own module (which is what per-app files do for `selfhost.users`), and that would drop the central list — but the contract would then only exist by assembly, and `submoduleWith` + `specialArgs` makes merging it from several places fiddly. The asymmetry with `users` is deliberate: a service contract is read far more often than it is extended, so it is worth keeping in one place. Adding a facet touches three spots (schema, implementation, the list). Leave it that way unless the contract stops fitting on a page.

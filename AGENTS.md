...
- Gate everything behind an `enable`: importing a module must change nothing until it's turned on. Its own toggle is the whole gate — there is no framework-wide `enable` above it, since every assertion the registry makes is vacuously true on a host that registered nothing.
...
- **An app is curated, not configurable-in-half**: `selfhost.apps.<name>.enable` runs the app *and* its framework wiring (users, storage, reconcilers). There is no flag to keep one without the other — the curation is the point, and a half-wired app is a shape nobody asked for. An app that genuinely needs a framework-free life gets a standalone base module under `services.<name>` instead (`filebrowser-quantum`). Cross-cutting per-service settings (`access.*`, `integrations.*`) stay independently settable on `selfhost.services.<name>`.
...
- Chapters explain a subsystem's *model* (the why/how), never its options. Options self-document via their `description`, and a reader reaches for the options page before a chapter, so prose that restates an option is worse than none. A new chapter is a `docs/src/<name>.md` plus a line in `SUMMARY.md`.

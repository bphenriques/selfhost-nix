...
- **Per-user namespace is a 1:1 mirror** (see the Users docs chapter): a per-user option lives at `selfhost.users.<name>.<path>` mirroring its top-level `selfhost.<path>` — an app's at `selfhost.users.*.apps.<name>`, a concern's at `selfhost.users.*.<concern>` (`auth.oidc`, `vpn.wireguard`). Don't invent a flat per-user key; mirror the path so it's discoverable.
